...
export function useStartupCoordinator(
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const preserveCompleted =
        hadPrior && !d.onboardingCompletionCommittedRef.current;
      ...
      if (!restored) {
        ...
        d.setOnboardingOptions({
          names: [],
          styles: getStylePresets(d.uiLanguage),
          providers: [
    ...
    (async () => {
      ...
      while (!cancelled && effectRunRef.current === runId) {
        ...
        if (Date.now() >= deadline) {
          d.setStartupError(describeBackendFailure(lastErr, true));
          d.setOnboardingLoading(false);
        ...
        try {
          ...
          if (auth.required && !client.hasToken()) {
            ...
            d.setPairingEnabled(auth.pairingEnabled);
            d.setPairingExpiresAt(auth.expiresAt);
            d.setStartupPhase("ready");
            d.setOnboardingLoading(false);
          ...
          let sessionComplete =
            complete ||
            d.onboardingCompletionCommittedRef.current ||
          ...
          if (complete && sessionComplete) {
            clearPersistedOnboardingStep();
            d.onboardingResumeConnectionRef.current = null;
          }
          if (
            sessionComplete &&
            !ctx?.persistedConnection &&
          ...
          if (!sessionComplete) {
            ...
            while (!cancelled && effectRunRef.current === runId) {
              if (Date.now() >= optDeadline) {
                d.setStartupError(describeBackendFailure(optErr, true));
                d.setOnboardingLoading(false);
                dispatch({ type: "BACKEND_TIMEOUT" });
                return;
              }
              try {
                ...
                d.setOnboardingOptions({
                  ...
                  styles:
                    options.styles.length > 0
                      ? options.styles
                      : getStylePresets(d.uiLanguage),
                });
                if (!rc) {
                  try {
                    const det = await scanProviderCredentials();
                    if (det.length > 0) d.applyDetectedProviders(det);
                  } catch {}
                }
                if (rf.onboardingRunMode !== undefined)
                  d.setOnboardingRunMode(
                    rf.onboardingRunMode as "local" | "cloud" | "",
                  );
                if (rf.onboardingCloudProvider !== undefined)
                  d.setOnboardingCloudProvider(rf.onboardingCloudProvider);
                if (rf.onboardingProvider !== undefined)
                  d.setOnboardingProvider(rf.onboardingProvider);
                if (rf.onboardingVoiceProvider !== undefined)
                  d.setOnboardingVoiceProvider(rf.onboardingVoiceProvider);
                if (rf.onboardingApiKey !== undefined)
                  d.setOnboardingApiKey(rf.onboardingApiKey);
                if (rf.onboardingPrimaryModel !== undefined)
                  d.setOnboardingPrimaryModel(rf.onboardingPrimaryModel);
                if (rf.onboardingOpenRouterModel !== undefined)
                  d.setOnboardingOpenRouterModel(rf.onboardingOpenRouterModel);
                if (rf.onboardingRemoteConnected !== undefined)
                  d.setOnboardingRemoteConnected(rf.onboardingRemoteConnected);
                if (rf.onboardingRemoteApiBase !== undefined)
                  d.setOnboardingRemoteApiBase(rf.onboardingRemoteApiBase);
                if (rf.onboardingRemoteToken !== undefined)
                  d.setOnboardingRemoteToken(rf.onboardingRemoteToken);
                if (rf.onboardingSmallModel !== undefined)
                  d.setOnboardingSmallModel(rf.onboardingSmallModel);
                if (rf.onboardingLargeModel !== undefined)
                  d.setOnboardingLargeModel(rf.onboardingLargeModel);
                d.setOnboardingStep(
                  inferOnboardingResumeStep({
                ...
                if (ae?.status === 401 && client.hasToken()) {
                  ...
                  d.setAuthRequired(true);
                  d.setPairingEnabled(latestAuth.pairingEnabled);
                  d.setPairingExpiresAt(latestAuth.expiresAt);
                  d.setStartupPhase("ready");
                  d.setOnboardingLoading(false);
                  dispatch({ type: "BACKEND_AUTH_REQUIRED" });
                  return;
                ...
                if (ae?.status === 404) {
                  d.setStartupError(describeBackendFailure(err, false));
                  d.setOnboardingLoading(false);
          ...
          if (ae?.status === 401 && client.hasToken()) {
            client.setToken(null);
            depsRef.current!.setAuthRequired(true);
            depsRef.current!.setPairingEnabled(latestAuth.pairingEnabled);
            depsRef.current!.setPairingExpiresAt(latestAuth.expiresAt);
            depsRef.current!.setStartupPhase("ready");
            depsRef.current!.setOnboardingLoading(false);
            dispatch({ type: "BACKEND_AUTH_REQUIRED" });
            return;
          }
          if (ae?.status === 404) {
            depsRef.current!.setStartupError(
              describeBackendFailure(err, false),
            );
            depsRef.current!.setOnboardingLoading(false);
            dispatch({ type: "BACKEND_NOT_FOUND" });
            return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      while (!cancelled && effectRunRef.current === runId) {
        const d = depsRef.current!;
        if (Date.now() >= deadline) {
          d.setStartupError(describeAgentFailure(lastErr, true, lastDiag));
          d.setOnboardingLoading(false);
        ...
        try {
          ...
          if (status.pendingRestart) {
            d.setPendingRestart(true);
            d.setPendingRestartReasons(status.pendingRestartReasons ?? []);
          }
          ...
          lastErr = err;
          depsRef.current!.setConnected(false);
        }
        await new Promise<void>((r) => {
          tid = setTimeout(r, 500);
        });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      d.setOnboardingLoading(false);
      if (greetConvId)
        void d.requestGreetingWhenRunningRef.current(greetConvId);
      ...
      // Wallet addresses
      try {
        d.setWalletAddresses(await client.getWalletAddresses());
      } catch (e) {
      ...
      try {
        ...
        if (typeof si === "number" && Number.isFinite(si)) {
          ...
          d.setSelectedVrmIndex(resolvedIdx);
        }
      } catch (e) {
      ...
      if (resolvedIdx === 0) {
        if (await client.hasCustomVrm())
          d.setCustomVrmUrl(resolveApiUrl(`/api/avatar/vrm?t=${Date.now()}`));
        else d.setSelectedVrmIndex(1);
        if (await client.hasCustomBackground())
          d.setCustomBackgroundUrl(
      ...
      const shouldCharSelect =
        ...
        shouldStartAtCharacterSelectOnLaunch({
          ...
          onboardingMode: d.onboardingMode,
          navPath,
          urlTab,
        });
      if (!d.initialTabSetRef.current) {
        ...
        if (shouldCharSelect) {
          d.onboardingCompletionCommittedRef.current = false;
          d.setTab("character-select");
          void d.loadCharacter();
      ...
      if (urlTab && urlTab !== "chat" && urlTab !== "companion") {
        ...
        if (urlTab === "plugins" || urlTab === "connectors") {
          void d.loadPlugins();
          if (urlTab === "plugins") void d.loadSkills();
        }
        if (urlTab === "settings") {
          void d.checkExtensionStatus();
          void d.loadWalletConfig();
          void d.loadCharacter();
          void d.loadUpdateStatus();
          void d.loadPlugins();
        }
