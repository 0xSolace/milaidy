...
export function CreditsPanel() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-32">
        <div className="text-brand font-mono text-sm animate-pulse">Loading credits...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <h3 className="font-mono text-xs uppercase tracking-widest text-brand">Credits</h3>

      {/* Balance card */}
      <div className="bg-dark border border-white/10 rounded p-6">
        <div className="text-text-muted font-mono text-[10px] uppercase tracking-wider mb-2">Balance</div>
      ...
      {session && (
        <div className="bg-dark border border-white/10 rounded p-6 space-y-4">
          <div className="text-text-muted font-mono text-[10px] uppercase tracking-wider">Current Session Usage</div>
          <div className="grid grid-cols-3 gap-6">
            <div>
              <div className="text-text-muted font-mono text-[10px] uppercase">Requests</div>
              <div className="text-text-light font-mono text-2xl mt-1">{session.requests?.toLocaleString() ?? "\u2014"}</div>
            </div>
            <div>
              <div className="text-text-muted font-mono text-[10px] uppercase">Tokens</div>
              <div className="text-text-light font-mono text-2xl mt-1">{session.tokens?.toLocaleString() ?? "\u2014"}</div>
            </div>
            <div>
              <div className="text-text-muted font-mono text-[10px] uppercase">Credits Used</div>
              <div className="text-text-light font-mono text-2xl mt-1">{session.credits?.toLocaleString() ?? "\u2014"}</div>
            </div>
          </div>
        </div>
      )}
