...
export function AgentDetail({ agent, managedAgent, connectionId, webUIUrl }: AgentDetailProps) {
  const [tab, setTab] = useState<Tab>("Overview");
  const [actionLoading, setActionLoading] = useState<string | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);

  const handleCloudAction = useCallback(async (action: string) => {
      ...
      {isCloud && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-4 border-t border-border">
          ...
          <InfoItem label="Region">
            <span className="text-text-light">{managedAgent.region ?? "Auto"}</span>
          </InfoItem>
          <InfoItem label="Created">
            <span className="text-text-light">{formatDate(managedAgent.createdAt)}</span>
          </InfoItem>
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-brand">
              ...
              {managedAgent.billing.currency && (
                <span className="text-xs text-text-muted ml-1">{managedAgent.billing.currency}</span>
              )}
            </p>
          </div>
          {managedAgent.billing.plan && (
            <div className="text-right">
              ...
              <p className="text-sm text-text-light font-medium">{managedAgent.billing.plan}</p>
            </div>
          )}
        </div>
      )}
...
function CloudActionButton({
  ...
  const colors = {
    ...
    default: "text-text-muted hover:text-text-light hover:bg-surface border-border",
  };
