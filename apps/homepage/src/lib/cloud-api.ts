...
export class CloudClient {
  ...
  async listAgents(): Promise<CloudAgentDetail[]> {
    const data = await this.request<any>("/api/v1/milady/agents", { method: "GET" });
  ...
  async createAgent(config: { name: string; characterId?: string; config?: object; environmentVars?: Record<string, string> }): Promise<{ id: string }> {
  ...
  async deleteAgent(agentId: string): Promise<void> {
    await this.request(`/api/v1/milady/agents/${agentId}`, { method: "DELETE" });
  }

  // Lifecycle
  async provisionAgent(agentId: string): Promise<{ jobId?: string }> {
    return this.request(`/api/v1/milady/agents/${agentId}/provision`, { method: "POST" });
  }

  async suspendAgent(agentId: string): Promise<void> {
    await this.request(`/api/v1/milady/agents/${agentId}/suspend`, { method: "POST" });
  }

  async resumeAgent(agentId: string): Promise<{ jobId?: string }> {
    return this.request(`/api/v1/milady/agents/${agentId}/resume`, { method: "POST" });
  }

  // Snapshots & backups
  async takeSnapshot(agentId: string): Promise<void> {
    await this.request(`/api/v1/milady/agents/${agentId}/snapshot`, { method: "POST" });
  }

  async listBackups(agentId: string): Promise<CloudBackup[]> {
    const data = await this.request<any>(`/api/v1/milady/agents/${agentId}/backups`, { method: "GET" });
    return Array.isArray(data) ? data : data.backups ?? data.data ?? [];
  }
  ...
  async bridge(agentId: string, method: string, params?: object): Promise<any> {
  ...
  async getAgentBridgeStatus(agentId: string): Promise<{ state: string; uptime?: number; memories?: number }> {
  ...
  async pollJobUntilDone(jobId: string, timeoutMs = 120000): Promise<JobStatus> {
  ...
  async listContainers(): Promise<any[]> {
    const data = await this.request<any>("/api/v1/containers", { method: "GET" });
    return Array.isArray(data) ? data : data.containers ?? data.data ?? [];
  }

  async getContainerHealth(containerId: string): Promise<object> {
    return this.request(`/api/v1/containers/${containerId}/health`, { method: "GET" });
  }

  async getContainerMetrics(containerId: string): Promise<object> {
    return this.request(`/api/v1/containers/${containerId}/metrics`, { method: "GET" });
  }

  async getContainerLogs(containerId: string): Promise<string> {
    const res = await fetch(`${CLOUD_BASE}/api/v1/containers/${containerId}/logs`, {
