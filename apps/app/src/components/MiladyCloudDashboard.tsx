        ...
        return (
            <div className="flex flex-col items-center justify-center h-full max-w-2xl mx-auto px-6 text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                >
                    ...
                    {miladyCloudLoginBusy ? t("miladyclouddashboard.Connecting") : t("miladyclouddashboard.ConnectMiladyCloud")}
                </Button>
                <p className="mt-6 text-sm text-muted/60">
                    {t("miladyclouddashboard.NewToMiladyCloud")}{" "}
                    <a href="https://miladycloud.ai" target="_blank" rel="noopener noreferrer" className="text-accent underline hover:text-accent-hover transition-colors">
        ...
        <div className="h-full overflow-y-auto custom-scrollbar p-6 lg:p-10 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-muted">{t("miladyclouddashboard.ManageInstance")}</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        {cloudDisconnecting ? t("miladyclouddashboard.Disconnecting") : t("miladyclouddashboard.Disconnect")}
                    </Button>
                </div>
            </div>
                    ...
                    >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                            ...
                            <div className="aspect-[4/3] rounded-2xl border border-dashed border-border/60 flex flex-col items-center justify-center p-6 text-center group hover:border-accent/50 hover:bg-accent/5 transition-all duration-300 cursor-pointer">
                                ...
                                <h3 className="font-bold text-txt-strong mb-1">{t("miladyclouddashboard.DeployNewAgent")}</h3>
                                <p className="text-xs text-muted">{t("miladyclouddashboard.InitializeInstance")}</p>
                            </div>
                        </div>
                    </SectionCard>
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-gradient-to-br from-accent/10 to-accent/5 border border-accent/20 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="mb-8">
                                <span className="text-[11px] text-muted uppercase font-bold tracking-wider block mb-1">{t("miladyclouddashboard.AvailableBalance")}</span>
                                <div className={`text-4xl font-bold tracking-tight flex items-baseline gap-1 ${creditStatusColor}`}>
                                    <span className="text-2xl opacity-70">$</span>
                                    {miladyCloudCredits !== null ? miladyCloudCredits.toFixed(2) : "0.00"}
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2">
                                    <Button variant="outline" className="flex-1 rounded-xl h-10 border-border/50 text-xs">
                    ...
                    >
                        <div className="space-y-4 mt-2">
                            <div className="p-3 rounded-2xl bg-bg/30 border border-border/30">
                                <span className="text-[10px] text-muted uppercase font-bold tracking-wider mb-1 block">{t("miladyclouddashboard.CloudUserID")}</span>
                                <code className="text-xs text-txt-strong break-all font-mono">{miladyCloudUserId || "Not available"}</code>
                            </div>

                            <div className="flex items-center justify-between p-3">
                                <div className="flex items-center gap-2">
                                    <Shield className="w-4 h-4 text-ok" />
                                    <span className="text-xs font-medium">{t("miladyclouddashboard.SecurityStatus")}</span>
                                </div>
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-ok/10 text-ok font-bold uppercase tracking-wider border border-ok/20">{t("miladyclouddashboard.Secure")}</span>
                            </div>

                            <Button
                                variant="link"
                                className="w-full text-xs text-accent justify-start px-3 h-auto"
                                onClick={() => window.open("https://miladycloud.ai/dashboard", "_blank")}
                            >
